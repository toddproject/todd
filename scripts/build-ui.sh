...
readonly SWAGGER_SRC="api/exp/swagger/swagger-ui/..."
readonly SWAGGER_PKG="swagger"

function kube::hack::build_ui() {
  local pkg="$1"
  local src="$2"
  local output_file="api/exp/swagger/ui/data/${pkg}/datafile.go"

  go-bindata -nocompress -o "${output_file}" -prefix ${PWD} -pkg "${pkg}" "${src}"

  local year=$(date +%Y)
  echo -e "// generated by scripts/build-ui.sh; DO NOT EDIT\n" >> "${TMP_DATAFILE}"
